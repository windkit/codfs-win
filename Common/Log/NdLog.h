...
#define logbasic(outputMode, fmt, ...) \
	do { \
	wprintf( L"[%04d]" _MODULE_NAME L"!" __FUNCTIONW__ L": " fmt L"\n", GetCurrentThreadId(), __VA_ARGS__);\
	__pragma (warning(push)) \
	__pragma (warning(disable:4127)) \
	} while (0) \
	__pragma (warning(pop))

#define logbasics(outputMode, fmt, ...) \
	do { \
	string fmt_str(fmt); \
	wstring fmt_str_w(fmt_str.begin(), fmt_str.end()); \
	fmt_str_w = L"[%04d]" _MODULE_NAME L"!" __FUNCTIONW__ L": " + fmt_str_w + L"\n"; \
	wprintf(fmt_str_w.c_str(), __VA_ARGS__); \
	__pragma (warning(push)) \
	__pragma (warning(disable:4127)) \
	} while (0) \
	__pragma (warning(pop))
...
#define logs(fmt, ...) logbasics(g_OutputLogFlag, fmt, __VA_ARGS__);
#endif

#define logfm(fmt, ...) logbasic(static_cast<LOG_OUTPUT_FLAG>(LOF_DEBUG_VIEW | LOF_FILE), fmt, __VA_ARGS__);
