...
int pread(unsigned int fd, char *buf, size_t count, int offset)
{
	...
	if ((cur_pos = _lseek(fd, 0, SEEK_CUR)) == (off_t)-1)
		return -1;
	...
	size_t numread = read(fd, buf, count);

	if (_lseek(fd, cur_pos, SEEK_SET) != offset) {
		return -1;
	}
...
int pwrite(unsigned int fd, char *buf, size_t count, int offset)
{
	...
	size_t numWritten = write(fd, buf, count);
